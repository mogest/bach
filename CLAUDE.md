...
- Has zero direct internet egress — the project network is `--internal`. Outbound HTTP/HTTPS goes through bach-proxy. One proxy serves the whole system: a dual-homed proxy container reachable as `bach-proxy:8080`. **Per-project hostname allowlist** (from `.bach.toml` `allowlist = [...]`), DNS-rebinding protection (refuses RFC1918 resolutions), pending-approval hold before rejection (default 10s; user-level `proxy_hold_seconds` → `BACH_HOLD_SECS` on the proxy container, recreate-on-change via label `bach.proxy.hold_seconds`), 10min temp-allow on approval, dashboard at `http://127.0.0.1:8081/`. Default-deny: no global allowlist — projects without a configured allowlist hold every request for manual approval.
...
- Gets a per-session **share dir**: host `~/.local/share/bach/shares/<session>/` mounted RW at `/shared` (mode 0777 — VirtioFS preserves host uid on bind mounts, so 0777 is what lets both the host user and agent/1000 write; no chown, no entrypoint involvement). NOT under /tmp: Docker Desktop's VirtioFS caches negative lookups there (a recreated /tmp path can stay "bind source path does not exist" to the VM) and macOS's temp cleaner could remove a live session's dir. The staged CLAUDE.md suffix tells claude to write user-facing output files there. `bach paste` (host side) saves the clipboard — image via osascript `«class PNGf»`/`«class TIFF»`, else text via pbpaste — into a running session's share dir as `paste-<timestamp>.<ext>` and pbcopy's the `/shared/...` path for pasting into claude. `bach send <file>` does the same for an arbitrary host file (name collision → `-<hex4>` suffix). `bach recv` picks an existing file from the share dir (newest-first picker mirroring `pick_session`: lone file auto-picked, fzf when available, else numbered prompt) — or watches for new files with `-w`/`--watch` (deliver every new file until ^C; snapshot names at start, poll 0.5s, wait for size to hold still one poll) or `-1`/`--once` (first new file only). Delivery: `-c` content to the host clipboard — text (utf-8, no NULs) via pbcopy, png/jpg/gif/tiff/pdf via osascript `set the clipboard to (read ... as «class X»)`, error on other binary; `-f` copy to `./<name>` (refuses to overwrite); `-F <filename>` copy to `<filename>` (overwrites — explicit name = intent); `-o` reveal in Finder via `open -R`. No flag → per-file c/f/o prompt (F not offered; non-TTY errors). Session picking mirrors `bach exec` (`--to` / `$BACH_SESSION` / picker). On session exit the dir is rmdir'd only if empty; non-empty dirs of dead sessions are reaped by `reap_share_dirs` on any later invocation once untouched for 7 days.
...
- `~/.local/share/bach/shares/<session>/` — per-session RW share dir (`/shared` in the container)
...
- bach POSTs each project's source CIDR + allowlist + denylist (+ port allocations for the dashboard) to the proxy dashboard endpoint `POST /project` (gated by the same dashboard-CIDR check) after every `ensure_proxy` call. The proxy keeps an in-memory `CIDR → Project{name, patterns, denylist, ports}` map and uses `r.RemoteAddr` to look up which project a request belongs to. Authorization order per request: allowlist → temp-allow → denylist (immediate reject, no pending hold) → temp-deny (immediate reject; planted for 60s / `BACH_TEMP_DENY_SECS` when a pending hold times out unanswered, so retries don't re-hold; cleared by dashboard approval, and expiry means the next request starts a fresh pending period) → pending. Expired temp-allows are dropped lazily on check, so the next request after expiry also re-pends. The dashboard exposes a `GET /ports` JSON view of the per-project allocations.
...
- `bach paste [--to <name>]` — save the host clipboard (text or image) as a file in a running session's `/shared` dir and copy the in-container path to the clipboard for pasting into claude.
- `bach send [--to <name>] <file>` — copy a host file into a running session's `/shared` dir and copy the in-container path to the clipboard. `bach paste` for files.
- `bach recv [--to <name>] [-w|-1] [-c|-f|-F <filename>|-o]` — fetch a file from a running session's `/shared` dir: pick from existing files (default), or watch for new ones (`-w` = every new file until ^C, `-1`/`--once` = just the first). Delivery: `-c` clipboard (text/image/pdf), `-f` copy to `./<name>`, `-F <filename>` copy to `<filename>`, `-o` reveal in Finder; no flag prompts c/f/o per file.
